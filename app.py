...
t = {
    "Korean (한국어)": {
        ...
        "t1_desc": "주류 물리학은 은하 외곽의 별들이 튕겨나가지 않고 빠르게 도는 현상을 **'암흑 물질(Dark Matter)'**이라는 가상의 입자로 설명합니다. 하지만 **K-PROTOCOL**에 따르면, 이는 입자가 아니라 은하 중심의 거대 질량 매듭이 만들어낸 **거시적 공간 굴절 텐서(S_loc)**의 기하학적 누적 현상입니다.",
        ...
        "t1_info": "💡 **결과 분석:** 계수 α가 K-PROTOCOL의 한계 차원(π³³)에서 유도된 **순수 기하학적 상수(1/22 ≈ 0.04545)**에 도달하면 실제 NASA 관측 데이터와 완벽히 일치하는 평탄한 곡선이 완성되며, 암흑 물질이라는 착시가 수학적으로 해결됩니다.",
        "t2_sub": "미시적 증명: 양자 얽힘은 '확률의 붕괴'가 아니라 단일 위상 매듭(T)의 투영이다.",
        "t2_desc": "주류 양자역학은 얽힘을 '빛보다 빠른 유령 같은 원격 작용'으로 보지만, **K-PROTOCOL**은 이를 **단일 π-매트릭스 꼬임(T)**의 3차원 직교 투영으로 해석합니다. 두 입자는 분리된 것이 아니라, 기하학적으로 연결된 하나의 끈입니다.",
    ...
    "English": {
        ...
        "t1_desc": "Standard physics explains the rapid rotation of stars at the galactic edge using hypothetical particles called **'Dark Matter'**. However, according to the **K-PROTOCOL**, this is not a particle, but a geometric accumulation of the **Macroscopic Spatial Refraction Tensor (S_loc)** generated by massive topological knots at the galactic center.",
        ...
        "t1_info": "💡 **Analysis:** When α reaches the pure geometric constant **1/22 (≈ 0.04545)** derived from the K-PROTOCOL's limit dimension (π³³), the curve aligns perfectly with NASA's observed data, mathematically resolving the Dark Matter illusion.",
        "t2_sub": "Microscopic Proof: Quantum Entanglement is a geometric projection of a single phase knot (T).",
        "t2_desc": "While standard quantum mechanics views entanglement as 'spooky action at a distance', **K-PROTOCOL** interprets it as a 3D orthogonal projection of a **single π-Matrix twist (T)**. The two particles are not separate, but a single topologically connected string.",
...
with tab1:
    ...
    fig1.update_layout(
        ...
        hovermode="x unified",
        # 범례를 그래프 밖 상단(가로형)으로 배치하여 겹침 원천 차단!
